/***********************************************
* Program: cox analysis 200714.SAS                          
* Folder: S:\Intmed_Rsrch2\GenMed\Restricted\BP COG\Aim 2\Dementia Model\SAS Progs 
* Author: Nick Tilton                             
* Created: 07/14/20                           
* Summary: Cox Model for time to incident dementia - 
*	 
***********************************************/
...
model (t_start,t_end)*deminc(0) = gcp_bl gcp_slope age0 female0 educ racebpcog;
...
/* produce hazard ratios within cohorts */

proc sort data=dem.coxreg_currgcp out=currgcp; by studyname newid t_end; run;

proc phreg data = currgcp;
by studyname;
class female0 (ref="Yes") racebpcog (ref="Non-Hispanic White") 
educ (ref="College graduate or more (Technical School Certificate, Associate Degree, Bachelor's Degree, Graduate or Professional School)");	   	
model (t_start,t_end)*deminc(0) = gcp_bl gcp_slope age female0 educ racebpcog;
...
/* Diagnostic models to test different time and age parameters */

data dem.coxreg_currgcp;
set dem.coxreg_currgcp;
tx_start=t_start+age0;
tx_end=t_end+age0;
run;

proc phreg data = dem.coxreg_currgcp;
title 'Approach 1 - baseline age, time from baseline';
class female0 (ref="Yes") racebpcog (ref="Non-Hispanic White") 
educ (ref="College graduate or more (Technical School Certificate, Associate Degree, Bachelor's Degree, Graduate or Professional School)");	   	
model (t_start,t_end)*deminc(0) = gcp_bl age0 female0 educ racebpcog;
run;

proc phreg data = dem.coxreg_currgcp;
title 'Approach 3 - time-varying age, time from baseline';
class female0 (ref="Yes") racebpcog (ref="Non-Hispanic White") 
educ (ref="College graduate or more (Technical School Certificate, Associate Degree, Bachelor's Degree, Graduate or Professional School)");	   	
model (t_start,t_end)*deminc(0) = gcp_bl age female0 educ racebpcog;
run;

proc phreg data = dem.coxreg_currgcp;
title 'Approach 2 - baseline age, time from age';
class female0 (ref="Yes") racebpcog (ref="Non-Hispanic White") 
educ (ref="College graduate or more (Technical School Certificate, Associate Degree, Bachelor's Degree, Graduate or Professional School)");	   	
model (tx_start,tx_end)*deminc(0) = gcp_bl age0 female0 educ racebpcog;
run;
